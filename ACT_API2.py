...
from API_record import capture_one_episode
import time
import cv2
...
class ACT_API:
    ...
    def run_record(self,config_file):
        try:
            capture_one_episode(config_file)
        except Exception as error:
            self.error_flag.set()
            self.inference_done.set()
            self.completed_event.set()
            if self._logger is not None:
                self._logger.error("Error in running record:",error)
            raise error
