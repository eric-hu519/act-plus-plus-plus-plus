...
def closing_ceremony(puppet_bot_left, puppet_bot_right,master_bot_left=None, master_bot_right=None):
    ...
    if master_bot_left == None:     #replay dont need master
        ...
        # delete:manual mode to adjust the CLOSE_ARM_POSE:
        index = 0
...
def capture_one_episode(dt, max_timesteps, camera_names, dataset_dir, dataset_name, overwrite):
    ...
    '''
    for i in range(6):
            print(f'Robot will be shutdown in {5-i} seconds')
            time.sleep(1)
    print(f'preparing for closing master and puppet')
    '''
    closing_ceremony(puppet_bot_left=env.puppet_bot_left,puppet_bot_right= env.puppet_bot_right,master_bot_left=master_bot_left,master_bot_right= master_bot_right)
    return True
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task_name', action='store', type=str, help='Task name.', default='aloha_right_grasp')#required=True)
