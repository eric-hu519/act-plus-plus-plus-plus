...
class RealEnv:
    ...
    def __init__(self, init_node, setup_robots=True, setup_base=False, is_record=False):
    ...
    def _reset_joints(self, opposite_dir=False):
        if self.is_record:
            from . import robot_utils
        else:
            import robot_utils
        reset_position = START_ARM_POSE[:6]
        robot_utils.move_arms([self.puppet_bot_left, self.puppet_bot_right], [reset_position, reset_position], move_time=1)
        if opposite_dir:
            #time.sleep(2)
            CLOSE_ARM_POSE_L = [-1.6, -0.96, 1.16, 0, -0.3, 0]
            CLOSE_ARM_POSE_R = [1.6, -0.96, 1.16, 0, -0.3, 0]
            robot_utils.move_arms([self.puppet_bot_left, self.puppet_bot_right], [CLOSE_ARM_POSE_L,CLOSE_ARM_POSE_R], move_time=1)
            #time.sleep(5)
    def _reset_gripper(self):
        ...
        robot_utils.move_grippers([self.puppet_bot_left, self.puppet_bot_right], [PUPPET_GRIPPER_JOINT_OPEN] * 2, move_time=0.5)
        robot_utils.move_grippers([self.puppet_bot_left, self.puppet_bot_right], [PUPPET_GRIPPER_JOINT_CLOSE] * 2, move_time=1)
    ...
    def get_observation(self, get_tracer_vel=False):
        ...
        if get_tracer_vel and self.use_base:
    ...
    def reset(self, fake=False, opposite_dir=False):
        if not fake:
            # Reboot puppet robot gripper motors
            self.puppet_bot_left.dxl.robot_reboot_motors("single", "gripper", True)
            self.puppet_bot_right.dxl.robot_reboot_motors("single", "gripper", True)
            self._reset_joints(opposite_dir)
            self._reset_gripper()
    ...
    def step(self, action, base_action=None, get_tracer_vel=False, get_obs=True, opposite_act = False):
        ...
        if opposite_act:
            left_action[0]= left_action[0]-1.6
            right_action[0]= right_action[0]+1.6
        #move_arms([self.puppet_bot_left, self.puppet_bot_right], [left_action[:6], right_action[:6]], move_time=5)

        self.puppet_bot_left.arm.set_joint_positions(left_action[:6],  moving_time=2,blocking=False)
        self.puppet_bot_right.arm.set_joint_positions(right_action[:6], moving_time=2, blocking=False)
        ...
        if get_obs:
            ...
            obs = None
        return dm_env.TimeStep(
            step_type=dm_env.StepType.MID,  #当前步骤为中间步骤
            reward=self.get_reward(),
            discount=None,
            observation=obs)
...
def make_real_env(init_node, setup_robots=True, setup_base=False,is_record=False):
    env = RealEnv(init_node, setup_robots, setup_base,is_record)
    return env
...
def test_real_teleop():
    ...
    # source of data  从两个主臂读取关节姿势
    master_bot_left = InterbotixManipulatorXS(robot_model="wx250s", group_name="arm", gripper_name="gripper",
                                              robot_name=f'master_left', init_node=True)
    master_bot_right = InterbotixManipulatorXS(robot_model="wx250s", group_name="arm", gripper_name="gripper",
                                               robot_name=f'master_right', init_node=False)
    robot_utils.setup_master_bot(master_bot_left)
    robot_utils.setup_master_bot(master_bot_right)
